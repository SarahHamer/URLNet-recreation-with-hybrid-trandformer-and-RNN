...
# Dataset (with ngram vocab + high_freq_words)
def build_dataset(args):
    ...
    # ngram vocab (influenced by high_freq_words)
    ...
    # char encoding (uses ngram_vocab)
    char_data = []
    for url in urls:
...
def batch_generator(data, labels, args, shuffle=True):
    ...
    while True:
        ...
        for i in range(0, len(indices), args.batch_size):
            ...
            if args.mode == 1:
                x = data["char"][batch_idx]
            elif args.mode == 2:
                x = data["word"][batch_idx]
            elif args.mode == 3:
                x = (data["char"][batch_idx], data["word"][batch_idx])
            else:
                x = (
                    data["char"][batch_idx],
                    data["word"][batch_idx]
...
def build_feed(model, batch_x, batch_y, args, training=True):
    ...
    if args.mode == 1:
        ...
        feed[model.input_char_seq] = batch_x[0]
        feed[model.input_word] = batch_x[1]
        feed[model.input_char] = batch_x[2]
        feed[model.input_char_mask] = np.ones_like(batch_x[2], dtype=np.float32)
...
def evaluate(sess, model, data, args, val_idx):
    ...
    for i in range(0, len(val_idx), args.batch_size):
        ...
        if args.mode == 1:
            x_batch = data["char"][batch_idx]
        elif args.mode == 2:
            x_batch = data["word"][batch_idx]
        elif args.mode == 3:
            x_batch = (
                data["char"][batch_idx],
                data["word"][batch_idx]
        ...
        feed = build_feed(model, x_batch, y_batch, args, training=False)
